    ...
    <View style={styles.container}>
      ...
      <Text style={styles.body}>
        {isConnectionIssue
          ...
          : `${profile?.email ?? "This account"} isn't set up for the Partner app.`}
      </Text>

      {isConnectionIssue && (
        <Pressable style={styles.retryButton} onPress={handleRetry} disabled={retrying}>
          {retrying ? <ActivityIndicator color={colors.white} size="small" /> : <Text style={styles.buttonText}>Retry</Text>}
        </Pressable>
      )}
...
const styles = StyleSheet.create({
  ...
  button: { borderWidth: 1, borderColor: colors.muted, borderRadius: 999, paddingHorizontal: 20, paddingVertical: 10 },
