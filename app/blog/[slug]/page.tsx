  ...
  return (
    ...
    >
      ...
      <div className="mt-12 rounded-2xl border border-nav-light-green bg-nav-mint p-6 text-center">
        ...
        <Link
          href="/#pickup"
          className="mt-3 inline-flex items-center justify-center rounded-full bg-nav-primary px-5 py-2.5 text-sm font-medium text-white! transition-colors hover:bg-nav-primary/90"
        >
          Request a Pickup
        </Link>
      </div>
          ...
          <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
            {related.map((r) => (
              <Link
                key={r.slug}
                href={`/blog/${r.slug}`}
                className="group flex flex-col rounded-2xl border border-black/5 bg-white p-5"
              >
